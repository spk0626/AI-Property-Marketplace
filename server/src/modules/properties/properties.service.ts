...
export class PropertiesService {
  ...
  async create(ownerId: string, dto: CreatePropertyDto) {
    ...
    this.logger.log(
      `Property created with ID: ${property.id} by User: ${ownerId}`,
    );
    return property;
  }
  ...
  async uploadImage(
    ...
    const uploaded = await new Promise<{
      // uploaded: a Promise that resolves to an object containing the secure URL and public ID of the uploaded image on Cloudinary
      secure_url: string;
      public_id: string;
    }>((resolve, reject) => {
