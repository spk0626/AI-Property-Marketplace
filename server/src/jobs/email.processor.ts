...
function isBookingConfirmationJob(
  value: unknown,
): value is BookingConfirmationJob {
  if (!value || typeof value !== 'object') return false;
  const payload = value as Record<string, unknown>;
  return (
    typeof payload.to === 'string' &&
    typeof payload.userName === 'string' &&
    typeof payload.propertyTitle === 'string' &&
...
export class EmailProcessor {
  ...
  async handleBookingConfirmation(
    ...
    if (!isBookingConfirmationJob(job.data)) {
      throw new Error('Invalid booking confirmation payload');
    }
